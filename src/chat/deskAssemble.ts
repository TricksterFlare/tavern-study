...
// 形状要求:衣装/关系按人物分键、未收伏笔按线索分键——前端状态板面板只有对象值才展开成
// 子行(TypingDesk boardToRows),写成一整句就只有孤行;位置/时间地点保持单句不拆。
// 存量窗口的旧板靠"本楼拆开重写一次"那两句在下一楼自然转换,别指望刷新钮转——刷新链路
// 的形状闸(新旧同档判据)会把跨档转换打回,这是刻意的:转换只走本楼层路径,转完之后那道闸
// 反过来替子行形状站岗。楼层路径另有协议闸兜底(core/stateBoard.ts stateBoardGateErrors):
// 缺协议键/丢输入键/协议键错形状整板弃用沿旧板——它校验目标形状不比新旧,所以不拦转换、只逼转换。
const STATEBOARD_INSTRUCTION =
  '（系统提示,不是台词)每楼正文结束后,请另起一段用 ```stateboard 围栏输出更新后的状态板 JSON,' +
  '键固定为在场角色/衣装/位置/关系/时间地点五项;「在场角色」写成字符串数组;「衣装」「关系」写成按人物分键的对象,每个人一个子键、值为一句话,不许把几个人挤进同一句;「位置」「时间地点」保持单句字符串;当前板上若有键还是挤成一整句的旧写法,本楼按此形状拆开重写一次。五项均按本楼结束时刻的实际状态如实更新——人物离场就从「在场角色」移除(仅被提及、回忆、口述的人物不计入在场),场景移动了「位置」「时间地点」要跟着走,更衣了「衣装」要改;某项这楼确实没有变化,内容就原样带一遍,不要省略这个块——但"原样"只指内容,形状若还是旧写法,内容再没变也要按上面的形状要求拆。' +
