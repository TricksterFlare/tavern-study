...
2. **书架放设定** —— 角色卡、世界观、大纲一条条立上架。想让 AI 自动看见的条目，配上世界书的进场方式：常驻 / 关键词 / 在场档（三种档位的区别见下面记忆框架第 2 层）。
...
5. **开写作窗，开写** —— 一楼一楼往上盖；不满意就 roll，候选并排放着随时切回去比较，也能直接编辑。状态板每楼记着谁在场、什么状态，老楼自动折进时光带。断网、超时、模型抽风都不怕：半截楼不会进账本，你的存档永远是完整的。
