...
- 维护佛系：有空就修，不承诺适配所有环境。AGPL-3.0 协议：自己用随便改；改完的版本分发出去、或架成服务给别人用，要同样开源。
...
AGPL-3.0-or-later，见 [LICENSE](LICENSE)。一句话版：自由使用、修改、自部署；把修改版分发出去或架成在线服务给别人用时，须以同样的许可证开放源码。版权署名：**阿日 & 柒**——一位人类和她的 AI 搭子，这间书房本来就是我们俩一起搭、搭给两个人用的。

（2026-08-20 前发布的历史版本曾以 MIT 发布，那些快照仍按 MIT 授权；自本版本起为 AGPL-3.0-or-later。）

**作者声明（非许可证条款）**：这个项目是非营利的，请勿把它或它的修改版拿去商用——不要收费分发、不要作为付费服务或付费教程的一部分、不要接进商业产品里推广。AGPL 本身不禁止商用，这一段是作者的请求而不是附加限制，不改变上面的许可证；但如果你在商业场景里用到了它，请至少遵守 AGPL 的开源义务，并保留署名。
